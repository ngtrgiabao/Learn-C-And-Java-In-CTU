...
int main(){
	...
	int *contro_int;
	contro_int=&mang_int[0][0]; // hoac nhap (int*)mang_int 
	...
	for (i=0; i<4; i++){
		printf("\n");
		for (j=0; j<5; j++) printf("%d\t",contro_int[5*i + j]);
	}
	for(i=0; i<20; i++) contro_int[i]++ ;
	printf("\n-----------------------------");
	...
	for (i=0; i<20; i++) printf("%d ",contro_int[i]);
	if (contro_int!=NULL) free(contro_int);
	return 0;
}
