...
//void PushDown(recordtype a[], int first, int last){ // min -> max [�oi dau trong if ( < && >)]
//	int r = first;
//	while(r <= (last-1)/2){
//		if(last == 2*r+1){
...
//void PushDown(recordtype a[], int first, int last){ // max -> min [�oi dau trong if ( > && <)]
//	int r = first;
//	while(r <= (last-1)/2){
//		if(last == 2*r+1){
//			if(a[r].key > a[last].key) swap(&a[r], &a[last]);
...
// Code ngan gon hon tren mang:
	void Heapify(recordtype a[], int n, int i){
	    ...
	    if (l<n && a[l].key < a[largest].key) largest = l; // doi ch� so sanh key
	    if (r<n && a[r].key < a[largest].key) largest = r; // doi ch� so sanh key
	    if (largest != i) {
	        swap(&a[i], &a[largest]);
	        Heapify(a, n, largest);
	    }
