...
void Nhanh_Can(int i, int x[], DoVat *dsdv, int n){
	 int j = V/dsdv[i].TL; // change 
	 TGT = TGT + j*dsdv[i].GT; // Tong gia tri hien co
	 V = V - j*dsdv[i].TL; // Trong luong con lai
	 ...
	 x[i] = 0;
	 TGT -= j*dsdv[i].GT;	// Tra lai Tong gia tri cua nut cha
	 V += j*dsdv[i].TL;		// Tra lai Trong luong con lai cua nut cha
}
