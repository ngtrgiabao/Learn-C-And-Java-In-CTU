...
void InDSDV(DoVat *dsdv , int n){
	...
	printf("|---|--------------------|---------|---------|---------|-----------|-----------|\n");
	for(i=0; i<n; i++){
		printf("|%2d |%-20s|%9.2f|%9.2f|%9.2f|%11d|%6d     |\n",
...
void Cap_Nhat_GLNTT(int x[], DoVat *dsdv, int n){
	 int i;
	 if(GLNTT < TGT){
		 GLNTT = TGT;
		 for(i=0;i<n;i++)
...
int min(int a, int b){ // khong dung
	return (a<b) ? a : b;
}

void Nhanh_Can(int i, int x[], DoVat *dsdv, int n){
	 int z, j;
	 j = min(dsdv[i].SL, V/dsdv[i].TL);
	 for (z = j; z>=0; z--){
		 TGT += z*dsdv[i].GT; // Tong gia tri hien co
		 V -= z*dsdv[i].TL; // Trong luong con lai
		 CT = TGT + V*dsdv[i+1].DG;	// dsdv[i+1].DG la DG vat ke tiep cua vat i (i + 1) // tuy cong thuc tinh can
		 if(CT > GLNTT){ 					// Truong hop Chua cat tia (Dieu kien Cat tia la khi CT <= GLNTT)
			x[i] = z;
		 	if(i==n-1 || !V) Cap_Nhat_GLNTT(x, dsdv, n);
		 	else Nhanh_Can(i+1, x, dsdv, n); //Xet nut con cua nut i
	 	 }	
		 // Quay lui xet nut khac
		 x[i] = 0;
		 TGT -= z*dsdv[i].GT;	// Tra lai Tong gia tri cua nut cha
		 V += z*dsdv[i].TL;		// Tra lai Trong luong con lai cua nut cha
	 }
}
...
int main(){
	...
	BubbleSort(dsdv, n); // Sap xep lai theo don gia
	Tao_Nut_Goc(dsdv[0].DG);
	Nhanh_Can(0, x, dsdv, n);
	InDSDV(dsdv, n);
	free(dsdv);
	return 0;
}
